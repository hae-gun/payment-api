...
| 문서 | 내용 |
|---|---|
| [상태 전이](docs/state-machine.md) | 결제 상태 6개와 허용·금지 전이, 전이별 테스트 대응표 |
| [멱등성 처리](docs/idempotency.md) | 승인 흐름의 멱등성, 동시 요청 처리, 트랜잭션 경계 판단 |
...
PG 응답이 없다는 건 요청이 도달하지 않았다는 뜻이 아닙니다. 이를 실패로
처리하면 실제로 승인된 거래를 실패로 알리게 되므로 UNKNOWN 상태를
따로 두었습니다.

→ [상태와 전이 규칙](docs/state-machine.md)

### 트랜잭션 경계

PG 호출을 트랜잭션 밖에 두고 앞뒤로 커밋을 나눴습니다. PG 호출은
롤백되지 않기 때문입니다.

→ [세 가지 이유](docs/idempotency.md#트랜잭션을-셋으로-나눈-이유)
...
## 테스트가 실제로 무엇을 잡는지 확인한 방법

커버리지만으로는 테스트가 무언가를 검증하는지 알 수 없어서,
구현을 한 번에 하나씩 망가뜨리고 어떤 테스트가 실패하는지 확인했습니다.

| 변이 | 깨진 테스트 | 무엇을 알려주나 |
|---|---|---|
| `canTransitionTo` 의 종료 상태 → `true` | 3 | 전이 검사가 approve/fail 경로를 지킴 |
| `isCancelable()` 제거 | 3 | 별도 관문. 2개는 메시지로만 잡힘 |
| 금액 상한 검사 제거 | 2 | 예외가 아예 안 남. 돈이 새는 결함 |
| `==` → `>=` | 0 | 등가 변이체. 테스트 문제 아님 |
| `List.copyOf` 제거 | 1 | 그 하나가 없으면 0. 커버리지 100%인데 무방비 |

여기서 두 가지를 고쳤습니다.

- 예외 타입만 검증하던 테스트에 메시지 검증을 추가했습니다.
  다른 관문이 같은 타입의 예외를 던져서, 검사를 지워도 통과하는 경우가 있었습니다.
- `isCancelable()` 과 `canTransitionTo` 가 같은 사실을 각각 들고 있어
  둘이 어긋나지 않는지 확인하는 테스트를 추가했습니다.

`==` 를 `>=` 로 바꾼 변이는 어떤 테스트로도 죽지 않습니다.
앞의 금액 검사가 그 조건을 도달 불가능하게 만들기 때문이고,
테스트를 추가할 일이 아니라고 판단했습니다.
